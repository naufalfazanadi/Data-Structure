...
void addFirst(char kode[],char nama[],list *L){
	if(countElement(*L) < n){
		int baru = emptyElement(*L);
		
		strcpy((*L).data[baru].elmt.kode, kode);
...
void addAfter(int prev,char kode[],char nama[],list *L){
...
void addLast(char kode[],char nama[],list *L){
	if((*L).first == -1){
		addFirst(kode,nama,L);
	}else{
		if(countElement(*L) < n){
			addAfter((*L).tail,kode,nama,L);
		}else{
			printf("sudah tidak dapat ditambahkan\n");
		}
	}
...
void printElement(list L){
	...
	if(L.first != -1){
		...
		while(elmt != -1){
			...
			printf("%s %s\n",L.data[elmt].elmt.kode,L.data[elmt].elmt.nama);
			//iterasi
			elmt = L.data[elmt].next;
			i++;
		}			
