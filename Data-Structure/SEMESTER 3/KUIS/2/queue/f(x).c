...
void keluarBarakah(int out, queue *Q) {
  /* INIT QUEUE FOR OUTPUT */
  queue QOut;
  createQueue(&QOut);

  elemen *var = (*Q).first; // init counter

  int i = 0; // counter too
  while (var != NULL && i < out) { // loop while not null and until num of out
    add(var->elmt, &QOut); // add to output queue
    del(Q); // del the main queue
  ...
  printQueue(QOut); // Output the out person
}